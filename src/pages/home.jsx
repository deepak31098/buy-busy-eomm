...
import {getAllProducts, queryProducts} from "../services/products";
...
export default function Products() {
  ...
  const [filterPrice, setFilterPrice] = useState(8000);
  ...
  useEffect(() => {
    showSpinner();
    const unsubscribe = getAllProducts(handleSnapShot);
    return () => unsubscribe();
  }, []);

  useEffect(() => {
    const query = {filterCategory, filterPrice};
    const unsubscribe = queryProducts(handleSnapShot, query);
    return () => unsubscribe();
